...
function onTradeRequest(cid, target, item)
	...
	if (getTileInfo(getThingPos(item.uid)).house) then
		local z = getHouseFromPos(getThingPos(item.uid))
		local master = getHouseOwner(z)
		local list = getHouseAccessList(z, 0x101)
		if (master == getPlayerGUIDByName(getCreatureName(cid))) or (string.find(list, getCreatureName(cid))) then
			else
			doPlayerSendTextMessage(cid, MESSAGE_INFO_DESCR, "Only the owner of the house can exchange items that are inside it.")
			return false
		end
	end
	if not isItemMovable(item.itemid) then
		doPlayerSendCancel(cid, "You can not trade this item!")
		return false
	end


	if getItemNameById(item.itemid):find("portrait") then
		doPlayerSendCancel(cid, "You Can't trade portraits.")
		return false
	end


	if #getCreatureSummons(cid) > 0 then
		doPlayerSendCancel(cid, "Back your pokemon.")
		return false
	end
	...
	if isContainer(item.uid) then
		...
		elseif getItemAttribute(item.uid, "unicc") then    --alterado v1.6
	...
	if isPokeball(item.itemid) then
		...
		-- START Pokemon Level and Gender System
		str = str.."•Name: "..name.."   •Boost: "..boost.."   •Level: "..level.."   •Exp: "..exps.."%   •Elite: "..elite
		-- END Pokemon Level and Gender System
...
local function noCap(cid, sid)
	if isCreature(cid) then
		doPlayerSendCancel(cid, "Sem lugar") 
	end
	if isCreature(sid) then
		doPlayerSendCancel(sid, "Sem lugar")
	end
end
