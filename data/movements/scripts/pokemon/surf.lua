...
function onStepIn(cid, item, position, fromPosition)
	...
	local pokeName = getPokemonName(getCreatureSummons(cid)[1])

	if isInArray(specialabilities["surf"], pokeName) and not surfs[pokeName] then
	...
	if getItemAttribute(item.uid, "boost") and getItemAttribute(item.uid, "boost") >= 50 and getPlayerStorageValue(cid, 42368) <= 0 then
		addEvent(sendAuraEffect, 120, cid, auras.getItemAttribute(item.uid, "aura") - 1)    --alterado v1.8
	end
