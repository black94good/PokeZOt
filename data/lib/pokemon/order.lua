...
function recheck(sid, skill, pos)
	...
	elseif skill == "headbutt" then
		...
		local array = {}
		local lvl = {25, 40, 60, 80, 150, 2500}
		...
		for j = 1, rand[2] do
			doCreateMonster(rand[1], getCreaturePosition(cid))
		end
		local item = getTileItemById(pos, 12591)
		doCreatureSay(sid, "HEADBUTT!", TALKTYPE_MONSTER)
		markPos(sid, {x=1,y=1,z=7})
		doFaceCreature(sid, pos)
		doSendMagicEffect(getThingPos(item.uid), 118)
		doTransformItem(item.uid, 12592)
		local function growHead()
			doTransformItem(getTileItemById(pos, 12592).uid, 12591)
		end
		addEvent(growHead, choose(5, 10, 20, 30) * 60 * 10)
	...
	elseif skill == "fly" then
		...
		if getCreatureOutfit(cid).lookType == 667 or getCreatureOutfit(cid).lookType == 999 then
			...
			sendMovementEffect(cid, 136, getThingPos(cid))
		end
	...
	if getOwnerPos(sid).x ~= getThingPos(getCreatureMaster(sid)).x or
	   getOwnerPos(sid).y ~= getThingPos(getCreatureMaster(sid)).y or
	   isCreature(getCreatureTarget(getCreatureMaster(sid))) then
		doRegainSpeed(sid)
		markPos(sid, {x=1,y=1,z=7})
		return true
	end
