	...
	const Tile* oldTile, const Position& oldPos, bool teleport)
{
	...
	else
	{
		...
		updateIdleStatus();
		if(!followCreature && (!isSummon() || isGuardian()) && isOpponent(creature)) //we have no target lets try pick this one
...
{
	if(isGuardian() && creature->getPlayer())
		return true;
...
{
	if(isGuardian() && (creature->isSummon() || creature->getPlayer()))
		return false;
	
	if(isGuardian() && getMaster() && getMaster()->getPlayer())
	{
		Player* mPlayer = getMaster()->getPlayer();
		if(creature->getPlayer() && mPlayer == creature->getPlayer())
			return false;
		else if(creature->isSummon() && creature->getMaster() && creature->getMaster()->getPlayer() == mPlayer)
			return false;
	}
	
	if(!creature->getPlayer() && creature->isGuardian())
		return true;
...
void Monster::onThinkTarget(uint32_t interval)
{
	...
	{
      	if (!searchTarget(TARGETSEARCH_RANDOM))
    ...
    //passive
    if (getAttackedCreature()->getPlayer() && getAttackedCreature()->getSummonCount() > 0)
	{
        const std::list<Creature*>& summons = getAttackedCreature()->getSummons();
        CreatureList::const_iterator it = summons.begin();
        selectTarget(*it);  
        return;
    }else if (getAttackedCreature()->getPlayer() && getAttackedCreature()->getGuardianCount() > 0)
	{
        const std::list<Creature*>& guardians = getAttackedCreature()->getGuardians();
