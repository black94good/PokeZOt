...
Creature::~Creature()
{
	...
	for(std::list<Creature*>::iterator cit = guardians.begin(); cit != guardians.end(); ++cit)
	{
		(*cit)->setAttackedCreature(NULL);
		(*cit)->setMaster(NULL);
		(*cit)->unRef();
...
void Creature::onRemovedCreature()
{
	...
	if(master && !master->isRemoved())
	{
		...
		//Viktor
		/* std::string dianSto;
        if(getStorage(80246, dianSto) && dianSto != "0" && atoi(dianSto.c_str()) == 1)
		    master->removeGuardian(this); */
	...
	for(CountMap::const_iterator it = damageMap.begin(); it != damageMap.end(); ++it)
	{
		...
		for(DeathList::iterator fit = list.begin(); fit != list.end(); ++fit)
		{
			...
			if(!(mdc->getName() == tmp->getName() && mdc->getMaster() == tmp->getMaster()) &&
				...
				&& (mdc->getGuardianCount() <= 0 || mdc->getSummonCount() <= 0 || tmp->getMaster() != mdc))
				continue;
...
void Creature::addGuardian(Creature* creature)
{
	creature->setDropLoot(LOOT_DROP_NONE);
	creature->setLossSkill(false);
    
	creature->setGuardian();
	creature->setMaster(this);
	creature->addRef();
	guardians.push_back(creature);
}

void Creature::removeGuardian(const Creature* creature)
{
	std::list<Creature*>::iterator it = std::find(guardians.begin(), guardians.end(), creature);
	if(it == guardians.end())
		return;
...
void Creature::destroyGuardians()
{
	for(std::list<Creature*>::iterator it = guardians.begin(); it != guardians.end(); ++it)
	{
		(*it)->setAttackedCreature(NULL);
		(*it)->changeHealth(-(*it)->getHealth());
