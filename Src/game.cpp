...
void Game::saveGameState(bool crash)
{
	...
	{
	    // Thalles Vitor - Adaptacao de Anti Roll Back
	    if (crash) {
			Town* town = Towns::getInstance()->getTown(it->second->getTown());
			if (!town)
		    	return;
...
bool Game::placeCreature(Creature* creature, const Position& pos, bool extendedPos /*= false*/, bool forced /*= false*/)
{
	...
	if (!getTile(pos))
	{
		...
		Tile* tile = IOMap::createTile(newItem, NULL, pos.x, pos.y, pos.z);
		setTile(tile);
		tile->onUpdateTile();
		internalAddItem(NULL, tile, newItem, INDEX_WHEREEVER, FLAG_NOLIMIT);
	}
	
	Tile* checkTile = map->getTile(pos);
	if (!checkTile->hasProperty(BLOCKSOLID) && !checkTile->ground) 
    {
        Item* newItemF = Item::CreateItem(460, 1);
        Tile* tileF = checkTile;
        tileF->onUpdateTile();
		internalAddItem(NULL, tileF, newItemF, INDEX_WHEREEVER, FLAG_NOLIMIT);
    }
...
ReturnValue Game::placeGuardian(Creature* creature, const std::string& name)
{
	Monster* monster = Monster::createMonster(name);
	if(!monster)
		return RET_NOTPOSSIBLE;
	...
	// Place the monster
	creature->addGuardian(monster);
	if(placeCreature(monster, getClosestFreeTile(monster,creature->getPosition(), false, false), true))
		return RET_NOERROR;
   	if(placeCreature(monster, getClosestFreeTile(monster,creature->getPosition(), true, false), true))
    	return RET_NOERROR;    

	creature->removeGuardian(monster);
	return RET_NOTENOUGHROOM;
}
...
bool Game::removeCreature(Creature* creature, bool isLogout /*= true*/)
{
	...
	for(std::list<Creature*>::iterator it = creature->guardians.begin(); it != creature->guardians.end(); ++it)
		removeCreature(*it);
...
ReturnValue Game::internalMoveCreature(Creature* creature, Direction direction, uint32_t flags/* = 0*/)
{
	...
	std::string flySto;
	if(creature->getPlayer() && creature->getStorage(17000, flySto) && atoi(flySto.c_str()) >= 1 && ret == RET_NOERROR && map->getTile(fromTile->getPosition()))
    {	
        Player* player = creature->getPlayer();
		Tile* tileFFly = getTile(fromTile->getPosition());
		Thing* thing = tileFFly->__getThing(0);
		if(thing)
		{
			Item* toItem = thing->getItem();
			if(thing->getItem() && toItem->getID() >= 1) 
			{
				if(toItem->getID() == 460 || toItem->getID() == 11676 || toItem->getID() == 11675 || toItem->getID() == 11677)
				{
					internalRemoveItem(NULL, thing->getItem());
					player->sendUpdateTileFly(fromTile->getPosition());
				}
			}
		}
    }
	...
	if(ret != RET_NOERROR)
	{
		...
		{
			std::string value;
			if((player->getStorage(17000, value) && atoi(value.c_str()) >= 1) && !map->getTile(destPos))
			{
				Item* newItem = Item::CreateItem(460);
				Tile* tile = IOMap::createTile(newItem, NULL, destPos.x, destPos.y, destPos.z);
				setTile(tile);
				
				Tile* checkTile = map->getTile(destPos);
				if (!checkTile->hasProperty(BLOCKSOLID) && !checkTile->ground) 
                {
                    Item* newItemF = Item::CreateItem(460, 1);
                    Tile* tileF = checkTile;
                    internalAddItem(NULL, tileF, newItemF, INDEX_WHEREEVER, FLAG_NOLIMIT);
					ReturnValue retF = RET_NOTPOSSIBLE;
                    retF = internalMoveCreature(NULL, creature, fromTile, toTile, flags);
					
					if(retF == RET_NOERROR)
					    tileF->onUpdateTile();
					
					 
                    if(map->getTile(fromTile->getPosition()))
                    {
						Tile* tileFFly = getTile(fromTile->getPosition());
						Thing* thing = tileFFly->__getThing(0);
					    if(thing)
			...
			}else if((player->getStorage(17000, value) && atoi(value.c_str()) >= 1) && map->getTile(destPos))
			{
				Tile* checkTile = map->getTile(destPos);
				if (!checkTile->hasProperty(BLOCKSOLID) && !checkTile->ground) 
                {
					Item* newItem = Item::CreateItem(460);
					Tile* tile = map->getTile(destPos);
					internalAddItem(NULL, tile, newItem, INDEX_WHEREEVER, FLAG_NOLIMIT);
					ReturnValue retF = RET_NOTPOSSIBLE;
                    retF = internalMoveCreature(NULL, creature, fromTile, toTile, flags);
	...
	uint16_t spriteId, int16_t fromStackpos, const Position& toPos, uint16_t count)
{
	...
	if(tileHoN)
	{
		...
		if(houseTileNo)
		{
			boost::any valueB = (item)->getAttribute("pokeH");
		    House* houseI = houseTileNo->getHouse();
		    if(houseI && valueB.type() == typeid(int32_t))
		    {
				uint32_t pokeH = (uint32_t)boost::any_cast<int32_t>(valueB);
				if (pokeH > 0)
				{
				    player->sendTextMessage(MSG_INFO_DESCR, "sorry, ball ativa.");
					return false;
				}
			}
   		}
	...
	int32_t index, Item* item, uint32_t count, Item** _moveItem, uint32_t flags /*= 0*/)
{
	...
	if(item->isStackable())
	{
		...
		{
			n = std::min((uint32_t)10000 - toItem->getItemCount(), m);
	...
	uint32_t flags /*= 0*/, bool test /*= false*/)
{
	...
	if(!test)
	{
		...
		{
			uint32_t n = std::min((uint32_t)10000 - toItem->getItemCount(), m);
			toCylinder->__updateThing(toItem, toItem->getID(), toItem->getItemCount() + n);
			if(m - n > 0)
			{
	...
	uint8_t index, uint16_t spriteId, bool isHotkey)
{
	...
	Tile* tileHoN = getMap()->getTile(pos);
	if(tileHoN)
	{
		HouseTile* houseTileNo = tileHoN->getHouseTile();
		if(houseTileNo)
		{
		    ...
		    House* houseI = houseTileNo->getHouse();
		    if(houseI && value.type() == typeid(std::string))
		    {
		    	House* houseO = Houses::getInstance()->getHouse(houseI->getId());
		        if(player->getGUID() != houseO->getOwner())
		        {
                    player->sendTextMessage(MSG_INFO_DESCR, "Sorry, but you can not use anything from this house.");
					return false;
				}
				
				std::string poke = boost::any_cast<std::string>(value);
				
				std::string strValue;
				player->getStorage(87612, strValue);
				if (atoi(strValue.c_str()) < 0)
				    player->setStorage(87612, "0");
				
				if(valueB.type() == typeid(int32_t))
				{
					uint32_t pokeH = (uint32_t)boost::any_cast<int32_t>(valueB);
					Creature* poHo = getCreatureByID(pokeH);
					if(poHo)
					{
                   		player->sendTextMessage(MSG_INFO_DESCR, "poke volte.");
						removeCreature(poHo);
						(item)->setAttribute("pokeH", 0);
   
						char buffer[100];
		                sprintf(buffer, "%d", atoi(strValue.c_str())-1);
						player->setStorage(87612, buffer);
				    ...
				    }else if(pokeH > 0)
					{
		                ...
		                sprintf(buffer, "%d", atoi(strValue.c_str())-1);
						player->setStorage(87612, buffer);
					    player->sendTextMessage(MSG_INFO_DESCR, "ball ativa.");
						return false;
				    }	
				}
				
				player->getStorage(87612, strValue);
				if (atoi(strValue.c_str()) >= 10)
				{
					player->sendTextMessage(MSG_INFO_DESCR, "sorry, voce não poke soltar mais poke.");
					return false;
				}
				
				Npc* npc = Npc::createNpc("PokeHouse");
				if(!npc)
                {
                    player->sendTextMessage(MSG_INFO_DESCR, "Sorry, fail.");
					return false;
				}     
                
				Position posP = pos;
				posP.x = 720; posP.y = 1084; posP.z = 7;
				if(!placeCreature(npc, posP))
				{
				    delete npc;
					player->sendTextMessage(MSG_INFO_DESCR, "Sorry, fail.");
					return false;
	            }
				const MonsterType* mType = g_monsters.getMonsterType(poke);
				if(mType)
				{
				    Spell::CreateIllusion((Creature*)npc, mType->outfit, -1);
				}
                
				Creature* cc = (Creature*)npc;
				internalTeleport(cc, pos, true);
				
				SpectatorVec list;
       			getSpectators(list, cc->getPosition());  
       			Player* playerS = NULL;

					const std::string* pokenick = item->getStringAttribute("nick");
					const int32_t* pokelevel = item->getIntegerAttribute("pokeLevel");
					const int32_t displayLevel = pokelevel ? *pokelevel : 1;
					if (pokenick) {
						std::ostringstream ss;
						ss << *pokenick << " [" << displayLevel << "]";

					cc->Nick = ss.str();
				}
				else {
					std::ostringstream ss;
						ss << poke << " [" << displayLevel << "]";

					cc->Nick = ss.str();
				}
				
				int32_t npp = cc->getID();
				(item)->setAttribute("pokeH", npp);
				...
				char buffers[300];
		        sprintf(buffers, "Você está vendo %s ele pertence a %s", poke.c_str(), player->getName().c_str());
				cc->setStorage(87612, buffers);

         		for(SpectatorVec::const_iterator it = list.begin(); it != list.end(); ++it)
        		{
           			if(playerS = (*it)->getPlayer())
               		{
                   		playerS->sendCreatureNick(cc);
               		}
        		}
				
				player->sendTextMessage(MSG_INFO_DESCR, poke);
...
bool Game::playerSetAttackedCreature(uint32_t playerId, uint32_t creatureId)
{
	...
	if(attackCreature->getNpc())
	{
		SpeakClasses type = SPEAK_SAY;
		Position* pos;
		//std::string value;
		const std::string& text = "hi";
		const Creature* creature = player->getCreature();
		attackCreature->getNpc()->onCreatureSay(creature, type, text, pos);//, Position* pos/* = NULL*/)
		player->setAttackedCreature(NULL);
		player->sendCancelTarget();
		return false;
	}
...
bool Game::playerTurn(uint32_t playerId, Direction dir)
{
	...
	if(player->getSummonCount() >= 1){
        std::list<Direction> dirs;
	    dirs.push_back(dir);
		const std::list<Creature*>& summons = player->getSummons();
	    CreatureList::const_iterator poke = summons.begin();
		Creature* pokemon = *poke;
		Position posp = getNextPosition(dir, pokemon->getPosition());
           if(pokemon->getMovePokemon()){
               player->setIdleTime(0);
               return pokemon->startAutoWalk(dirs);
           }
    }
...
return true;
	...
	if(player->getMsgCheck() == asLowerCaseString(text) && player->getMsgCount() == 1)
	{
		player->sendTextMessage(MSG_STATUS_SMALL, "You have already sent this message 2 times in a row.");
		return false;
	}
	player->setMsgPlayer(asLowerCaseString(text));
...
bool Game::doPlayerAddItemStacking(Player* player, Container* container, uint16_t itemid, uint8_t quant) 
{
	for(ContainerIterator it = container->begin(), end = container->end(); it != end; ++it)
    {
        Item* item = (*it)->getItem();//container->getItem(slot);
        if(item && item->getID() == itemid)
        {
            if(item->isStackable() && item->getItemCount() < 100)
            {
            	uint16_t count = item->getItemCount()+quant;
            	if(count > 10000)
            	   count = 10000;
            	 
            	transformItem(item, item->getID(), count);
            	if(item->getItemCount()+quant > 10000)
            	{
            		Item* newItem = Item::CreateItem(item->getID(), (item->getItemCount()+quant)-10000);
					internalPlayerAddItem(NULL, player, newItem, true);
				}
				return true;
            }
		}
	...
	Item* newItem = Item::CreateItem(itemid, quant);
	internalPlayerAddItem(NULL, player, newItem, true);
