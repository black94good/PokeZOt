	...
	OperatingSystem_t operatingSystem, uint16_t version, bool gamemaster)
{
	...
	{
		disconnectClient(0x14, "Your character has been blockend.");
		return false;
	}
...
bool ProtocolGame::connect(uint32_t playerId, OperatingSystem_t operatingSystem, uint16_t version)
{
	...
	if (!getConnection()) //no longer exists, so we return to prevent leakage of the Player.
		return false;
...
bool ProtocolGame::parseFirstPacket(NetworkMessage& msg)
{
	...
	ConnectionManager::getInstance()->addAttempt(getIP(), protocolId, true);//svke
	if(ConnectionManager::getInstance()->isDisabled(getIP(), protocolId))//svke
	{
		getConnection()->close();
		return false;
	}
	...
	if(!RSA_decrypt(msg))
	{
		disconnectClient(0x14, "Voc� s� pode logar com o Client Oficial.");
		return false;
	}
	...
	if (character.empty() || password.empty() || name.empty())
	{
		disconnect();
		return false;
	}
...
void ProtocolGame::parsePacket(NetworkMessage &msg)
{
	...
	else
	{
		...
		/* 		hex << "0x" << std::hex << (int16_t)recvbyte << std::dec; */
				s << player->getName() << " sent unknown byte: 0x" << (int16_t)recvbyte << std::endl;
...
void ProtocolGame::GetTileFlyDescription(const Tile* tile, NetworkMessage_ptr msg)
{
	...
	int32_t count = 0;
	if(!tile->hasProperty(BLOCKSOLID) && !tile->ground)
	{
		msg->AddItemId(460);
	    count++;
	}
	...
	if(creatures)
	{
		for(CreatureVector::const_reverse_iterator cit = creatures->rbegin(); (cit != creatures->rend() && count < 10); ++cit)
		{
			...
			bool known;
			uint32_t removedKnown;
			checkCreatureAsKnown((*cit)->getCreature()->getID(), known, removedKnown);
...
{
	if(!receivePacket(PACKET_ITEM, 750))
		return;
...
void ProtocolGame::sendAnimatedText(const Position& pos, uint16_t color, std::string text)
{
	if(!canSee(pos))
		return;
