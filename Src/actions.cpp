...
ReturnValue Actions::internalUseItem(Player* player, const Position& pos, uint8_t index, Item* item, uint32_t creatureId)
{
	...
	if(Container* container = item->getContainer())
	{
		...
		}else
		{
			...
			std::string lootAuto; 
	        player->getCreature()->getStorage(90996, lootAuto);
			if(container->getCorpseOwner() && lootAuto == "0099090909")
			{
				std::stringstream s;
				PositionEx posC = pos;
				//std::cout << "wang viado" << std::endl;
				for(int n = 1; n <= 255; n++)
            	{
		            posC.stackpos = n;
		            Tile* tile = g_game.getTile(posC.x, posC.y, posC.z);
					if(!tile)
						continue;
					
		            Thing* thing = tile->__getThing(posC.stackpos);
		            if (!thing || !thing->getItem())
                        continue;
							
                    Item* corpItem = thing->getItem();
					Container* corpCont = corpItem->getContainer();
					
                    if(corpItem->getContainer() && corpCont->getCorpseOwner())
                    {
                    	if(corpCont->getCorpseOwner() && player->canOpenCorpse(corpCont->getCorpseOwner())
			            && g_config.getBool(ConfigManager::CHECK_CORPSE_OWNER))
			            {
			            	for(int slot = corpCont->size(); slot >= 0; slot--)
				            {
                                Item* item = corpCont->getItem(slot);
					            if(item)
					            {
					            	boost::any cnn = item->getAttribute("poke");
					            	if(!cnn.empty())
					            	   continue;
						   
					            	g_game.internalRemoveItem(NULL, item, item->getItemCount());
					            	Thing* thinc = player->__getThing(3);
					            	Item* tigh = thinc->getItem();
					                g_game.doPlayerAddItemStacking(player, tigh->getContainer(), item->getID(), item->getItemCount());
					                s << "," << item->getClientID() << "," << item->getItemCount();
					            }	
			    	        }
					    }
					}
                }
				//player->sendExtendedOpcode(185, s.str());
			}
