...
class Creature : public AutoId, virtual public Thing
{
	...
	public:
		...
		bool getMovePokemon() const {return movpok;}
		int8_t gym;
        std::string Nick;
		...
		void setHideHealth(bool v) {hideHealth = v;}
		 bool movpok;
		void setMovePokemon(bool v) {movpok = v;}
		...
		virtual void addGuardian(Creature* creature);
		virtual void removeGuardian(const Creature* creature);
		void destroyGuardians();
		const std::list<Creature*>& getGuardians() {return guardians;}
		uint32_t getGuardianCount() const {return guardians.size();}
