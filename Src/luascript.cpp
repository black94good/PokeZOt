...
bool LuaScriptInterface::loadDirectory(const std::string& dir, Npc* npc/* = NULL*/)
{
	...
	for(boost::filesystem::directory_iterator it(dir), end; it != end; ++it)
	{
		...
		// Boost antigo usado pelo Stian's Dev-C++ retorna std::string em leaf().
		std::string s = it->leaf();
		if(!boost::filesystem::is_directory(it->status()) && (s.size() > 4 ? s.substr(s.size() - 4) : "") == ".lua")
			 files.push_back(s);
	}
...
void LuaScriptInterface::registerFunctions()
{
	...
	//doPlayerRemoveAutoLootItem(cid, item_name)
	...
	lua_register(m_luaState, "getPlayerAutoLootList", LuaScriptInterface::luaDoPlayerGetAutoLootList);

	//doPlayerEnabledAutoLoot(cid, option)
	lua_register(m_luaState, "doPlayerEnabledAutoLoot", LuaScriptInterface::luaDoPlayerEnabledAutoLoot);

	//getAutoLootEnabled(cid)
	lua_register(m_luaState, "getAutoLootEnabled", LuaScriptInterface::luaDoPlayerGetAutoLootEnabled);
	...
	//isGuardian(cid)
	lua_register(m_luaState, "isGuardian", LuaScriptInterface::luaIsGuardian);
	...
	//getMovePokemon(cid)
	lua_register(m_luaState, "getMovePokemon", LuaScriptInterface::luaGetMovePokemon);

	//setMovePokemon(cid)
	lua_register(m_luaState, "setMovePokemon", LuaScriptInterface::luaSetMovePokemon);
...
int32_t LuaScriptInterface::luaDoCreatureSetNick(lua_State* L)
{
    ...
    if(creature)
    {
        ...
        if(creature->getMonster() && creature->getStorage(1000, levelValue))
        {
            int32_t level = atoi(levelValue.c_str());
            if(level > 0 && displayNick.find(" [Lv. ") == std::string::npos)
            {
                std::stringstream levelSuffix;
                levelSuffix << " [Lv. " << level << "]";
...
int32_t LuaScriptInterface::luaDoPlayerAddItem(lua_State* L)
{
	...
	while(itemCount > 0)
	{
		int32_t stackCount = std::min(10000, subType);
...
int32_t LuaScriptInterface::luaIsGuardian(lua_State* L)
{
	//isGuardian(cid)
	ScriptEnviroment* env = getEnv();
	...
	lua_pushboolean(L, creature->isGuardian());
	return 1;
}
...
int32_t LuaScriptInterface::luaGetMovePokemon(lua_State* L)
{
	//getMovePokemon(cid)
	ScriptEnviroment* env = getEnv();

	if(Creature* creature = env->getCreatureByUID(popNumber(L)))
		lua_pushboolean(L, creature->getMovePokemon());
	else
	{
		errorEx(getError(LUA_ERROR_CREATURE_NOT_FOUND));
		lua_pushboolean(L, false);
...
int32_t LuaScriptInterface::luaSetMovePokemon(lua_State* L)
{
	//setMovePokemon(cid)
	ScriptEnviroment* env = getEnv();
    bool pkto = popNumber(L);
	if(Creature* creature = env->getCreatureByUID(popNumber(L))){
		creature->setMovePokemon(pkto);
		lua_pushboolean(L, true);
	}else
	{
		errorEx(getError(LUA_ERROR_CREATURE_NOT_FOUND));
...
{
	//doPlayerAddAutoLootItem(cid, item_name)
	std::string item_name = popString(L);
	ScriptEnviroment* env = getEnv();
	Player* player = env->getPlayerByUID(popNumber(L));
	if(!player)
	{
	...
	player->addAutoLootItem(item_name);
	lua_pushboolean(L, true);
	return 1;
}
...
int32_t LuaScriptInterface::luaDoPlayerRemoveAutoLootItem(lua_State* L)
{
	...
	player->removeAutoLootItem(item_name);
	lua_pushboolean(L, true);
	return 1;
}

int32_t LuaScriptInterface::luaDoPlayerGetAutoLootList(lua_State* L)
{
	//getPlayerAutoLootList(cid)
	ScriptEnviroment* env = getEnv();
    ...
    lua_newtable(L);
	std::list<std::string>::iterator sit = player->loot.begin();
	for(uint32_t i = 1; sit != player->loot.end(); ++sit, ++i)
	{
        lua_pushnumber(L, i);
...
int32_t LuaScriptInterface::luaDoPlayerEnabledAutoLoot(lua_State* L)
{
	//doPlayerEnabledAutoLoot(cid, option)
	bool option = popNumber(L);
	ScriptEnviroment* env = getEnv();
	Player* player = env->getPlayerByUID(popNumber(L));
	if(!player)
	{
	...
	player->setEnabledAutoLoot(option);
	lua_pushboolean(L, true);
	return 1;
}

int32_t LuaScriptInterface::luaDoPlayerGetAutoLootEnabled(lua_State* L)
{
	//getAutoLootEnabled(cid)
	ScriptEnviroment* env = getEnv();
	Player* player = env->getPlayerByUID(popNumber(L));
	...
	lua_pushboolean(L, player->getEnabledAutoLoot());
	return 1;
}
