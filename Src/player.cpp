 	...
 	for(int32_t i = SLOT_LAST-1; i >= SLOT_FIRST; --i)
	{
		...
		{ 
            ...
            if(value.type() == typeid(std::string))
            {
				...
				{
                    boost::any levepo = (item)->getAttribute("level");
                    boost::any sexopo = (item)->getAttribute("gender");
                    boost::any hpPo = (item)->getAttribute("hp");
					int32_t level = 0;
					int32_t sexo = 0;
					int32_t hpPok = 0;
					if(levepo.type() == typeid(int32_t))
						level = boost::any_cast<int32_t>(levepo);
						
					if(sexopo.type() == typeid(int32_t))
						sexo = boost::any_cast<int32_t>(sexopo);
					...
					{
						if(level <= 0)
						    sd << "," << foto->clientId << "|" << pokename << "|" << bo << "|" << boost::any_cast<int32_t>(sexo) << "|" << hpPok;
					    else							
						    sd << "," << foto->clientId << "|" << "[" << level << "] " << pokename << "|" << bo << "|" << boost::any_cast<int32_t>(sexo) << "|" << hpPok;
                	...
                	if(level <= 0)
						s << "," << foto->clientId << "|" << pokename << "|" << bo << "|" << boost::any_cast<int32_t>(sexo) << "|" << hpPok;
					else							
						s << "," << foto->clientId << "|" << "[" << level << "] " << pokename << "|" << bo << "|" << boost::any_cast<int32_t>(sexo) << "|" << hpPok;
                ...
                for(ContainerIterator it = container->begin(), end = container->end(); it != end; ++it)
      	        {
                    ...
                    if(value.type() == typeid(std::string))
                    {
				        ...
				        if(tapot.type() == typeid(int32_t))
				        {
							boost::any levepo = (*it)->getAttribute("level");
                    		boost::any sexopo = (*it)->getAttribute("gender");
	...
	uint32_t flags) const
{
	...
	if(destItem)
	{
		if(destItem->isStackable() && item->getID() == destItem->getID())
			maxQueryCount = 10000 - destItem->getItemCount();
		else
			maxQueryCount = 0;
	}
	else
...
void Player::setMsgPlayer(const std::string& msg)
{
	if(msgCheck == msg)
	{
		msgCount = 1;
